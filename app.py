...
file_names = os.listdir(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'plugins', ''))
module_names = [f"plugins.{file_name[:-3]}" for file_name in file_names if os.path.isfile(
    os.path.join(os.path.dirname(os.path.abspath(__file__)), 'plugins', '', file_name))]
...
def main():
    ...
    if 'webhook' in request.args:
        ...
        if checkwebhook(webhook_url) is not True:
            return Response(status=400, response="Your webhook URL is incorrect. Are you sure you entered it correctly?")
    else:
    ...
    try:
        ...
        return Response(status=405, response="That event isn't supported by Githook (yet).")
    except:
...
if __name__ == '__main__':
    ...
    # We'll still let them run if this is missing, it's not like it's gonna ruin everything. Unlike Karen. Please I just want my kids back
        ...
        print(
            "You are missing a 'PORT' key in your config. Defaulting to port 5000.",
        )
        config.PORT = 5000
    else:
